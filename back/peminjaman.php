...
if (isset($_POST["penjagaId"]) && isset($_POST["penjagaToken"]) && isset($_POST["shelterId"]) && isset($_POST["mahasiswaNim"]) && isset($_POST["sepedaId"])){
	...
	if (mysqli_num_rows($sql)==0){
        ...
        $cari = mysqli_query($link, "SELECT sepedaId, sepedaLokasi FROM sepeda WHERE sepedaId = '$sepedaId'");
    	$row1 = mysqli_fetch_array($cari, MYSQLI_ASSOC);
    	$sepedaShelter = $row1['sepedaLokasi'];
    	...
    	else if ($sepedaShelter !== $shelterId){
			$response["success"] = 0;
			$response["message"] = "Gagal, Sepeda Tidak di Shelter ini";
